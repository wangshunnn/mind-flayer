...
The desktop composer footer shows user turns, completed conversation model steps, non-cached input, output, cache reads, the latest step cache-hit percentage, and current context as `0.7%/1.0M`. Estimated context keeps its `~` prefix; unknown context uses `?`. Turns count distinct stored user messages; regeneration adds steps but not turns, and compaction does not add conversation steps. Persisted `stepCount` handles approval continuations; older messages use SDK step boundaries (or one step for legacy usage without boundaries). Totals include inactive regenerated replies and separately recorded compaction usage, merged by message/event ID. Compaction does not replace the latest-step hit rate. New optional message metadata (`lastStepUsage`, `lastStepAt`, `stepCount`, `cacheDetailsIncomplete`) survives the existing JSON persistence. Legacy missing usage is not invented.
