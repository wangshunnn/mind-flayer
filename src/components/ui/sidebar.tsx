...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // No persistence: keep sidebar state in memory only.
    },
    [setOpenProp, open]
  )
    ...
    >
      ...
      >
        <div
          data-sidebar="sidebar"
          data-slot="sidebar-inner"
          className="bg-transparent flex h-full w-full flex-col"
        >
          {children}
        </div>
      </div>
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "m-1.5 rounded-md sm:peer-data-[variant=inset]:peer-data-[state=expanded]:ml-0",
        className
      )}
      {...props}
    />
...
function SidebarGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="group"
      className={cn("relative flex w-full min-w-0 flex-col p-2 py-0.5", className)}
      {...props}
    />
  )
}
...
const sidebarMenuButtonVariants = cva(
  "cursor-pointer peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-lg p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4.5 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "hover:bg-sidebar-accent hover:text-sidebar-accent-foreground font-[350]",
        outline:
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "items-center justify-center rounded-sm p-0 outline-hidden transition-transform [&>svg]:size-4 [&>svg]:shrink-0",
        // Increases the hit area of the button on mobile.
        "after:absolute after:-inset-2 sm:after:hidden",
